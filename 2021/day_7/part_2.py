...
def main():
    ...
    for numb_1 in numbers.keys():
        ...
        for numb_2, count_2 in numbers.items():
            temp_fuel_needed += count_2*(abs(numb_2-numb_1)*(abs(numb_2-numb_1)+1)/2)
        if temp_fuel_needed < least_fuel_position:
            least_fuel_position = temp_fuel_needed

    print(least_fuel_position)
    # or
    print(min([sum([count_2*(abs(numb_2-numb_1)*(abs(numb_2-numb_1)+1)/2) for numb_2, count_2 in numbers.items()]) for numb_1 in numbers.keys()]))
