...
def is_double(password):
    """
    Check if the given password contains at least one pair of adjacent digits that are the same and not part of a larger group.
    
    Args:
        password (list): A list of digits representing a password.

    Returns:
        bool: True if there is a pair of adjacent digits that are the same and not part of a larger group, otherwise False.
    """
    # Use Counter to count the occurrences of each digit in the password
    counter = Counter(password)
    # Return True if there's a digit that occurs exactly twice, otherwise return False
...
def get_valid_passwords(min, max):
    ...
    1. It contains at least one pair of adjacent digits that are the same and not part of a larger group.
...
if __name__ == "__main__":
    ...
    print("Part 2: ", len(get_valid_passwords(*map(int, raw.split("-")))))
